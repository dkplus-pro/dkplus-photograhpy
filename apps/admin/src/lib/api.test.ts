...
describe("admin API client auth headers", () => {
  ...
  it("saves uploaded logo CDN URLs through a persisted brand path without auto endpoints", async () => {
    const uploadedLogoUrl =
      "https://cdn.example.test/brands/olympus-corporation/white.svg";
    const uploadedDarkLogoUrl =
      "https://cdn.example.test/brands/olympus-corporation/dark.svg";
    const autoEndpointHits: string[] = [];

    const fetchMock = vi.fn(async (input: unknown, init?: RequestInit) => {
      const url = String(input);
      if (/\/brands\/auto-[^/]+/.test(url)) {
        autoEndpointHits.push(url);
        return jsonResponse({ error: "Auto-derived brands are UI-only" }, 404);
      }
      ...
      if (url.endsWith("/brands") && init?.method === "POST") {
        ...
        expect(body.name).toBe("Olympus Corporation");
        expect(body.logoUrls).toEqual([uploadedLogoUrl]);
        return jsonResponse(
          {
            brand: {
      ...
      if (
        url.endsWith("/brands/olympus-corporation") &&
        init?.method === "PATCH"
      ) {
        const body = JSON.parse(String(init.body)) as {
          logoUrls?: string[];
        };
        expect(body.logoUrls).toContain(uploadedDarkLogoUrl);
        return jsonResponse({
          brand: {
            id: "olympus-corporation",
            name: "Olympus Corporation",
            ...
            aliases: ["OM System"],
            logos: [
              { url: uploadedLogoUrl, alt: "Olympus white" },
              {
                url: uploadedDarkLogoUrl,
                fileName: "dark.svg",
                alt: "Olympus dark",
              },
            ],
            logoUrls: [uploadedLogoUrl, uploadedDarkLogoUrl],
          },
        });
      }

      if (url.endsWith("/brands/olympus-corporation") && !init?.method) {
        return jsonResponse({
          brand: {
            id: "olympus-corporation",
            name: "Olympus Corporation",
