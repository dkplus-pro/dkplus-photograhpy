...
export const formatExposure = (seconds?: number | string): string | undefined => {
...
export const exifLine = (exif?: PhotoExif): string => {
  ...
  const details = [camera, exif.lens, exif.aperture, exif.shutter, exif.iso ? `ISO ${exif.iso}` : undefined]
    .filter(Boolean)
    .join(' · ');
  return details || 'EXIF pending';
};

export const photoTitle = (photo: PhotoRecord): string => photo.title?.trim() || `Untitled ${photo.id}`;

export const summarizeUpload = (previews: UploadPreview[]): string => {
  if (previews.length === 0) return 'No files staged';
  const topics = new Set(previews.map((preview) => preview.topicId).filter(Boolean));
  return `${previews.length} file${previews.length === 1 ? '' : 's'} staged${
    topics.size ? ` across ${topics.size} topic${topics.size === 1 ? '' : 's'}` : ''
  }`;
};
