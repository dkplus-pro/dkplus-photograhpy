...
export const extractExif = async (file: File): Promise<PhotoExif> => {
  try {
    ...
    return {
      ...
      aperture: formatAperture(readNumber(raw, ['FNumber', 'ApertureValue']) ?? readString(raw, ['FNumber', 'ApertureValue'])),
      shutter: formatExposure(readNumber(raw, ['ExposureTime', 'ShutterSpeedValue']) ?? readString(raw, ['ExposureTime', 'ShutterSpeedValue'])),
    ...
    return {
      ...
      cameraModel: error instanceof Error ? error.message : 'Unknown parse error',
    };
  }
};
