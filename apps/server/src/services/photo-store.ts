...
function normalizeGallery(value: unknown): GalleryData {
  if (Array.isArray(value)) {
    return { photos: value as PhotoRecord[], topics: [], updatedAt: new Date().toISOString() };
  }
  if (!isRecord(value)) {
    return emptyGallery();
  }
  return {
    ...
    topics: Array.isArray(value.topics) ? (value.topics as GalleryData['topics']) : [],
...
export class PhotoStore {
  ...
  async read(): Promise<GalleryData> {
    try {
      ...
      if (error instanceof SyntaxError) {
        throw new AppError(500, 'DATA_FILE_INVALID_JSON', `Data file contains invalid JSON: ${this.filePath}`);
      }
      throw error;
    }
  }
  ...
  async create(input: PhotoInput): Promise<PhotoRecord> {
    ...
    await this.mutate((data) => {
      if (data.photos.some((photo) => photo.id === next.id)) {
        throw new AppError(409, 'PHOTO_ID_CONFLICT', `Photo ${next.id} already exists`);
      }
  ...
  async batchDelete(ids: string[]): Promise<{ deleted: PhotoRecord[]; missing: string[] }> {
    let result: { deleted: PhotoRecord[]; missing: string[] } = { deleted: [], missing: [] };
