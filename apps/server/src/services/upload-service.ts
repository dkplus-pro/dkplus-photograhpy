...
function buildObjectKey(prefix: string, fileName: string): string {
  ...
  return [prefix.replace(/^\/+|\/+$/g, ''), year, month, `${Date.now()}-${randomUUID()}-${sanitizeFileName(fileName)}`]
    .filter(Boolean)
    .join('/');
}
...
function assertCosConfigured(cos: CosConfig): asserts cos is CosConfig & {
  ...
  if (!cos.secretId || !cos.secretKey || !cos.bucket || !cos.region) {
    throw new AppError(500, 'COS_NOT_CONFIGURED', 'COS is enabled but COS_SECRET_ID, COS_SECRET_KEY, COS_BUCKET, or COS_REGION is missing');
  }
}
...
export class UploadService {
  ...
  async store(file: UploadedFile): Promise<StoredUpload> {
    ...
    if (!file.mimetype.startsWith('image/')) {
      throw new AppError(400, 'UPLOAD_UNSUPPORTED_TYPE', 'Only image uploads are supported');
    }

    return this.config.cos.enabled ? this.storeInCos(file) : this.storeLocally(file);
  }
  ...
  private async storeInCos(file: UploadedFile): Promise<StoredUpload> {
    assertCosConfigured(this.config.cos);
    const key = buildObjectKey(this.config.cos.prefix, file.originalname);
    const { default: COS } = await import('cos-nodejs-sdk-v5');
    const client = new COS({ SecretId: this.config.cos.secretId, SecretKey: this.config.cos.secretKey });
    ...
    return {
      buffer: file.buffer,
      image: {
        url: joinPublicUrl(this.config.cos.cdnBaseUrl, key),
        key,
        fileName: file.originalname,
        mimeType: file.mimetype || mime.lookup(file.originalname) || 'application/octet-stream',
        size: file.size,
