...
function readDate(value: unknown, field: string): string | undefined {
  ...
  if (Number.isNaN(date.valueOf())) {
    throw new AppError(400, 'VALIDATION_ERROR', `${field} must be an ISO-compatible date`);
  }
...
function readImage(value: unknown): Partial<PhotoImage> | undefined {
  ...
  if (size !== undefined && (!Number.isFinite(size) || size < 0)) {
    throw new AppError(400, 'VALIDATION_ERROR', 'image.size must be a non-negative number');
  }
  const storage = readString(value.storage, 'image.storage');
  if (storage && !['local', 'cos', 'remote'].includes(storage)) {
    throw new AppError(400, 'VALIDATION_ERROR', 'image.storage must be local, cos, or remote');
  }
  return { url, key, fileName, mimeType, size, storage: storage as PhotoImage['storage'] | undefined };
}

export function validatePhotoInput(value: unknown, options: { requireImage?: boolean } = {}): PhotoInput {
  if (!isRecord(value)) {
...
export function validateIds(value: unknown): string[] {
  if (!isRecord(value) || !Array.isArray(value.ids)) {
    throw new AppError(400, 'VALIDATION_ERROR', 'ids must be provided as an array');
  }
  const ids = value.ids;
  if (ids.length === 0 || ids.some((id) => typeof id !== 'string' || !id.trim())) {
    throw new AppError(400, 'VALIDATION_ERROR', 'ids must contain one or more non-empty strings');
  }
