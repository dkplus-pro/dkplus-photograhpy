...
import { UploadService, type UploadedFile } from '../services/upload-service.js';
...
export function createPhotosRouter(store: PhotoStore, uploads: UploadService): Router {
  ...
  router.post('/photos', async (ctx) => {
    const photo = await store.create(validatePhotoInput(body(ctx), { requireImage: true }));
    ctx.status = 201;
    ctx.body = { photo };
  });

  router.put('/photos/:id', async (ctx) => {
    const photo = await store.update(ctx.params.id, validatePhotoInput(body(ctx)));
    ctx.body = { photo };
  });
  ...
  router.post('/uploads', upload.any(), async (ctx) => {
    ...
    if (incoming.length === 0) {
      throw new AppError(400, 'UPLOAD_REQUIRED', 'Provide one or more multipart image files');
    }
