  ...
  for (const photo of photos) {
    ...
    for (const topicId of topicIds) {
      if (!topics.has(topicId)) {
        topics.set(topicId, {
          id: topicId,
          title: photo.topicId === topicId && photo.topicTitle
            ? photo.topicTitle
            : topicId,
        });
      }
...
const normalizePhoto = (photo: RawPhoto, index: number): ResolvedPhoto => {
  ...
  const topicIds = Array.isArray(photo.topicIds)
    ...
    : photo.topicId
      ...
      : [];
  const takenAt =
    photo.takenAt ?? photo.exif?.capturedAt ?? photo.createdAt ?? new Date(0).toISOString();
  ...
  return compact({
    ...
    urls: {
      ...
      preview: photo.urls?.preview ?? asset.preview ?? asset.thumbnail ?? asset.original,
    },
...
export const normalizePayload = (payload: unknown): GalleryPayload => {
  ...
  const sourceTopics = Array.isArray(raw.topics)
    ? raw.topics.map(normalizeTopic).filter((topic): topic is Topic => Boolean(topic))
    : [];
  const topics = sourceTopics.length ? sourceTopics : deriveTopics(sourcePhotos);
