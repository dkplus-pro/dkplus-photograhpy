...
- Main watermark export renders a bottom-to-top gradient overlay, omits title text entirely, writes camera model as model-only metadata without the camera brand/make prefix, and allows the Logo selector to remain on “不使用 Logo” so no logo block or separator is drawn.
...
- Brand Logo file uploads use the generic asset upload route under `/api/uploads/assets`; the Admin client then merges the returned assets into the existing brand `logos`/`logoUrls` and saves through normal brand PATCH. This preserves multi-logo editing without a brand-specific multipart upload route.
...
- [ ] Brand Logo uploads use `/api/uploads/assets`, do not create gallery photo records, and are merged into brand `logos`/`logoUrls` via brand PATCH while preserving multiple Logo rows.
- [ ] Main watermark export omits title text, defaults to the no-logo option, hides the logo block/separator when no Logo is selected, uses model-only camera metadata, and renders the overlay as a bottom-to-top gradient.
